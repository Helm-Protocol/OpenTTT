...
class TurnkeyClient {
    ...
    apiKeyStamper, passkeyStamper, walletManager) {
            ...
            return withTurnkeyErrorHandling(async () => {
                ...
                if (walletAccount.source === WalletSource.Connected) {
                    ...
                    if (!addEthereumPrefix && isEthereum) {
                        throw new TurnkeyError("Connected Ethereum wallets automatically prefix messages. Use `addEthereumPrefix: true`.", TurnkeyErrorCodes.SIGN_MESSAGE_ERROR);
                    }
            ...
            return withTurnkeyErrorHandling(async () => {
                if (walletAccount.source === WalletSource.Connected) {
                    ...
                    switch (walletAccount.chainInfo.namespace) {
                        ...
                        default:
                            throw new TurnkeyError("Connected wallets do not support signAndSendTransaction for this transaction type.", TurnkeyErrorCodes.SIGN_AND_SEND_TRANSACTION_ERROR);
                    }
                }
                // this is an embedded wallet account
                ...
                if (!rpcUrl) {
                    throw new TurnkeyError("Missing rpcUrl: embedded wallets require an RPC URL to broadcast transactions.", TurnkeyErrorCodes.SIGN_AND_SEND_TRANSACTION_ERROR);
                }
        ...
        this.refreshSession = async (params) => {
            ...
            if (!session) {
                throw new TurnkeyError(`No active session found: ${sessionKey}`, TurnkeyErrorCodes.NO_SESSION_FOUND);
            }
            if (!this.httpClient) {
        ...
        this.verifyAppProofs = async (params) => {
            ...
            return withTurnkeyErrorHandling(async () => {
                ...
                const organizationId = organizationIdFromParams || session?.organizationId;
                if (!organizationId) {
                    throw new TurnkeyError("Organization ID is required to verify app proofs.", TurnkeyErrorCodes.INVALID_REQUEST);
                }
                if (!appProofs || appProofs.length === 0) {
                    throw new TurnkeyError("'appProofs' is required and cannot be empty.", TurnkeyErrorCodes.INVALID_REQUEST);
                }
                let lastPublicKey;
                let lastBootProof;
                for (const appProof of appProofs) {
                    if (!appProof.publicKey) {
                        throw new TurnkeyError("App proof publicKey is missing.", TurnkeyErrorCodes.INVALID_REQUEST);
                    }
                    let bootProof;
