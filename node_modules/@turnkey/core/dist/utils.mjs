import { TurnkeyErrorCodes, TurnkeyNetworkError, TurnkeyError } from '@turnkey/sdk-types';
import { Chain, Curve, WalletSource, StamperType } from './__types__/enums.mjs';
import { TurnkeyRequestError } from './__types__/error.mjs';
import { decodeBase64urlToString, uint8ArrayFromHexString, uint8ArrayToHexString, bs58 } from '@turnkey/encoding';
...
import { uncompressRawPublicKey, fromDerSignature } from '@turnkey/crypto';
...
function addressFromPublicKey(chain, publicKey) {
    if (chain === Chain.Ethereum) {
        ...
        if (publicKeyBytes.length === 65 && publicKeyBytes[0] === 0x04) {
            ...
            const publicKeyUncompressed = uncompressRawPublicKey(publicKeyBytes, Curve.SECP256K1);
...
async function sendSignedRequest(signedRequest) {
    ...
    if (!res.ok) {
        ...
        throw new TurnkeyNetworkError("Signed request failed", res.status, TurnkeyErrorCodes.BAD_RESPONSE, errorText);
    }
    return res.json();
}
/**
    ...
    if (customErrorsByMessages &&
        ...
        Object.keys(customErrorsByMessages).forEach((key) => {
            if (errorMessage.includes(key)) {
                throw new TurnkeyError(customErrorsByMessages[key].message, customErrorsByMessages[key].code, error);
            }
        });
    }
};
...
async function assertValidP256ECDSAKeyPair(pair) {
    ...
    if (!(privateKey instanceof CryptoKey) || !(publicKey instanceof CryptoKey)) {
        throw new TurnkeyError("Both keys must be CryptoKey instances.", TurnkeyErrorCodes.INVALID_REQUEST);
    }
    if (privateKey.type !== "private")
        throw new TurnkeyError("privateKey.type must be 'private'.", TurnkeyErrorCodes.INVALID_REQUEST);
    if (publicKey.type !== "public")
        throw new TurnkeyError("publicKey.type must be 'public'.", TurnkeyErrorCodes.INVALID_REQUEST);
    // Verify extractability and usages
    if (privateKey.extractable !== false) {
        throw new TurnkeyError("Provided privateKey must be non-extractable.", TurnkeyErrorCodes.INVALID_REQUEST);
    }
    if (!privateKey.usages.includes("sign")) {
        throw new TurnkeyError("privateKey must have 'sign' in keyUsages.", TurnkeyErrorCodes.INVALID_REQUEST);
    }
    if (!publicKey.usages.includes("verify")) {
        throw new TurnkeyError("publicKey must have 'verify' in keyUsages.", TurnkeyErrorCodes.INVALID_REQUEST);
    }
    ...
    if (pAlg.name !== "ECDSA" || pubAlg.name !== "ECDSA") {
        throw new TurnkeyError("Keys must be ECDSA keys.", TurnkeyErrorCodes.INVALID_REQUEST);
    }
    if (pAlg.namedCurve !== "P-256" || pubAlg.namedCurve !== "P-256") {
        throw new TurnkeyError("Keys must be on the P-256 curve.", TurnkeyErrorCodes.INVALID_REQUEST);
    }
    ...
    if (rawPub.length !== 65 || rawPub[0] !== 0x04) {
        throw new TurnkeyError("Public key must be an uncompressed P-256 point (65 bytes, leading 0x04).", TurnkeyErrorCodes.INVALID_REQUEST);
    }
    ...
    if (!ok) {
        throw new TurnkeyError("publicKey does not match privateKey (verify failed).", TurnkeyErrorCodes.INVALID_REQUEST);
    }
}
function isValidPasskeyName(name) {
    ...
    if (!nameRegex.test(name)) {
        throw new TurnkeyError("Passkey name must be 1-64 characters and only contain letters, numbers, spaces, dashes, underscores, colons, or slashes.", TurnkeyErrorCodes.INVALID_REQUEST);
    }
    return name;
}
...
function getClientSignatureMessageForLogin({ verificationToken, sessionPublicKey = undefined, }) {
    try {
        ...
        throw new TurnkeyError("Failed to create client signature bundle for login", TurnkeyErrorCodes.UNKNOWN, error);
    }
}
function getClientSignatureMessageForSignup({ verificationToken, email, phoneNumber, apiKeys, authenticators, oauthProviders, }) {
    try {
        ...
        throw new TurnkeyError("Failed to create client signature bundle for signup", TurnkeyErrorCodes.UNKNOWN, error);
    }
}
/**
...
export { addressFormatConfig, addressFromPublicKey, assertValidP256ECDSAKeyPair, broadcastTransaction, buildSignUpBody, createWalletAccountFromAddressFormat, decodeVerificationToken, fetchAllWalletAccountsWithCursor, findWalletProviderFromAddress, generateRandomBuffer, generateWalletAccountsFromAddressFormat, getActiveSessionOrThrowIfRequired, getAuthProxyConfig, getAuthenticatorAddresses, getClientSignatureMessageForLogin, getClientSignatureMessageForSignup, getCurveTypeFromProvider, getEncodedMessage, getEncodingType, getHashFunction, getPolicySignature, getPublicKeyFromStampHeader, getSignatureSchemeFromProvider, googleISS, hexSignedTxToBase58, isEthereumProvider, isReactNative, isSolanaProvider, isValidPasskeyName, isWalletAccountArray, isWeb, mapAccountsToWallet, parseSession, sendSignedRequest, splitSignature, toExternalTimestamp, withTimeout, withTimeoutFallback, withTurnkeyErrorHandling };
