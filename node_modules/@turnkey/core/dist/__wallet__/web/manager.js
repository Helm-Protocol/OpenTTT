...
class WebWalletManager {
    ...
    constructor(cfg) {
        ...
        if (enableNativeEvm) {
            this.wallets[enums.WalletInterfaceType.Ethereum] = new ethereum.EthereumWallet();
            this.addChainInterface(enums.Chain.Ethereum, enums.WalletInterfaceType.Ethereum);
        }
        // set up native Solana wallet support
        if (enableNativeSol) {
            this.wallets[enums.WalletInterfaceType.Solana] = new solana.SolanaWallet();
            this.addChainInterface(enums.Chain.Solana, enums.WalletInterfaceType.Solana);
        }
        ...
        if (cfg.walletConnect && enableWalletConnect) {
            ...
            const wcUnified = new base.WalletConnectWallet(this.wcClient, () => this.ensureWalletConnectReady(), { ethereumNamespaces, solanaNamespaces });
            this.wallets[enums.WalletInterfaceType.WalletConnect] = wcUnified;
            // add async init step to the initializer queue
            this.initializers.push(() => utils.withTimeout(wcUnified.init(), 15000, "WalletConnect wallet").catch((err) => {
                ...
                this.removeWalletInterface(enums.WalletInterfaceType.WalletConnect);
            }));
            // register WalletConnect as a wallet interface for each enabled chain
            if (enableWalletConnectEvm)
                this.addChainInterface(enums.Chain.Ethereum, enums.WalletInterfaceType.WalletConnect);
            if (enableWalletConnectSol)
                this.addChainInterface(enums.Chain.Solana, enums.WalletInterfaceType.WalletConnect);
        }
     ...
     */
    async ensureWalletConnectReady() {
     ...
     */
    removeWalletInterface(type) {
        delete this.wallets[type];
        for (const chain of Object.keys(this.chainToInterfaces)) {
            const filtered = (this.chainToInterfaces[chain] ?? []).filter((interfaceType) => interfaceType !== type);
