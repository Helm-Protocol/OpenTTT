...
class CrossPlatformWalletStamper {
    ...
    defaultInterface() {
        ...
        if (initializedInterfaces.includes(enums.WalletInterfaceType.Ethereum)) {
            return enums.WalletInterfaceType.Ethereum;
        }
        if (initializedInterfaces.includes(enums.WalletInterfaceType.Solana)) {
            return enums.WalletInterfaceType.Solana;
        }
        if (initializedInterfaces.includes(enums.WalletInterfaceType.WalletConnect)) {
            return enums.WalletInterfaceType.WalletConnect;
        }
...
class WalletStamper {
    ...
    async stamp(payload, provider) {
        ...
        let publicKey;
        try {
            signature = await this.wallet.sign(payload, provider, enums.SignIntent.SignMessage);
        }
        catch (error) {
            throw new Error(`Failed to sign the message: ${error}`);
        }
        const scheme = utils.getSignatureSchemeFromProvider(provider);
        try {
            ...
            else if (utils.isSolanaProvider(provider)) {
