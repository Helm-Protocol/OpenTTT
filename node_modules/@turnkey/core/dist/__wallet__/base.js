...
async function createWalletManager(cfg) {
    if (utils.isReactNative()) {
        const { MobileWalletManager } = await Promise.resolve().then(function () { return require('./mobile/manager.js'); });
        const manager = new MobileWalletManager(cfg);
        await manager.init(cfg);
        return manager;
    }
    else if (utils.isWeb()) {
        const { WebWalletManager } = await Promise.resolve().then(function () { return require('./web/manager.js'); });
