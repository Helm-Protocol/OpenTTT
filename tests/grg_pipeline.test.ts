...
describe("GrgPipeline", () => {
  it("should roundtrip encode/decode", () => {
    ...
    const shards = GrgPipeline.processForward(data);
    expect(shards.length).toBe(6); // 4 data + 2 parity

    const recovered = GrgPipeline.processInverse(shards, data.length);
  ...
  it("should reject oversized input", () => {
    ...
    expect(() => GrgPipeline.processForward(data)).not.toThrow("exceeds MAX_INPUT_SIZE");
  });
  ...
  it("should throw on length mismatch", () => {
    ...
    expect(() => GrgPipeline.processInverse(shards, 999)).toThrow("Length mismatch");
  });
