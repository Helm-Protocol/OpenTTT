...
export class PoTCTLog {
   ...
   */
  async queryAnchors(filter: { startTime?: number; endTime?: number; stratum?: string; limit?: number }): Promise<PoTAnchorEntry[]> {
    const { startTime, endTime, stratum, limit = 100 } = filter;
    
    const whereClause = [];
    if (startTime) whereClause.push(`timestamp_gte: "${startTime}"`);
    if (endTime) whereClause.push(`timestamp_lte: "${endTime}"`);
    if (stratum) whereClause.push(`stratum: "${stratum}"`);
    ...
    return json.data.poTAnchors.map((anchor: any) => ({
      ...anchor,
   ...
   */
  async getAuditTrail(txHash: string): Promise<PoTAnchorEntry | null> {
    const query = `
      query {
   ...
   */
  async getBuilderScore(builderAddress: string): Promise<{ totalAnchors: number; turboRate: number; avgLatencyMs: number }> {
   ...
   */
  async getNetworkStats(): Promise<{ totalAnchors: number; uniqueBuilders: number; avgTurboRate: number }> {
    const query = `
      query {
        poTAnchors(first: 1) {
          id
